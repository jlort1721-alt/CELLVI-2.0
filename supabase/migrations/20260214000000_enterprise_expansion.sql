...
-- Set search path to ensure PostGIS types are visible
...
-- Ensure columns exist if table already existed from partial migration
ALTER TABLE public.maintenance_logs ADD COLUMN IF NOT EXISTS odometer_reading DOUBLE PRECISION;
ALTER TABLE public.maintenance_logs ADD COLUMN IF NOT EXISTS cost DOUBLE PRECISION;
ALTER TABLE public.maintenance_logs ADD COLUMN IF NOT EXISTS scheduled_at TIMESTAMP WITH TIME ZONE;
ALTER TABLE public.maintenance_logs ADD COLUMN IF NOT EXISTS completed_at TIMESTAMP WITH TIME ZONE;
...
-- Ensure columns exist for fuel_logs too
ALTER TABLE public.fuel_logs ADD COLUMN IF NOT EXISTS gallons DOUBLE PRECISION;
ALTER TABLE public.fuel_logs ADD COLUMN IF NOT EXISTS cost_per_gallon DOUBLE PRECISION;
ALTER TABLE public.fuel_logs ADD COLUMN IF NOT EXISTS total_cost DOUBLE PRECISION;
ALTER TABLE public.fuel_logs ADD COLUMN IF NOT EXISTS odometer_reading DOUBLE PRECISION;

-- RLS para nuevas tablas
ALTER TABLE public.maintenance_logs ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.fuel_logs ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Tenants can view own maintenance" ON public.maintenance_logs;
CREATE POLICY "Tenants can view own maintenance" ON public.maintenance_logs
    FOR SELECT USING (tenant_id = get_user_tenant_id());

DROP POLICY IF EXISTS "Tenants can manage own maintenance" ON public.maintenance_logs;
CREATE POLICY "Tenants can manage own maintenance" ON public.maintenance_logs
    FOR ALL USING (tenant_id = get_user_tenant_id());

DROP POLICY IF EXISTS "Tenants can view own fuel logs" ON public.fuel_logs;
CREATE POLICY "Tenants can view own fuel logs" ON public.fuel_logs
    FOR SELECT USING (tenant_id = get_user_tenant_id());

DROP POLICY IF EXISTS "Tenants can manage own fuel logs" ON public.fuel_logs;
...
BEGIN 
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='geofences' AND column_name='geom') THEN
        -- Explicitly use extensions schema for GEOGRAPHY type just in case
        ALTER TABLE public.geofences ADD COLUMN geom extensions.GEOGRAPHY(POLYGON, 4326);
    END IF;
END $$;
...
BEGIN
    ...
    LOOP
        ...
        -- Uses functions from extensions schema implicitly via search_path setting below
